...
function CombinedModelAnimation({ onLoadComplete }) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          onUpdate: (self) => {
            ...
            } else if (self.progress >= 0.29 && self.progress <= 0.36) {
      ...
      lensElements.forEach((part, i) => {
        tl.to(part.position, { z: [0.02, 0.035, 0.05, 0.06, 0.07, 0.01][i] || 0 }, "lens-explode");
      });
      ...
      tl.addLabel("rotate-and-scalee")
        .to(cameraModelPosition.current, { x: 5, y: 3.5, z: 0 }, "rotate-and-scale")
        .to(
          cameraModelRotation.current,
          {
            x: 0,
      ...
      tl.addLabel("final-pose")
        .to(cameraModelPosition.current, { x: 0, y: 6, z: 0 }, "final-pose")
        .to(
          cameraModelRotation.current,
          {
            x: 0,
      ...
      tl.addLabel("final-posee")
        ...
        .to(cameraModelPosition.current, { x: 0, y: 9, z: 0 }, "final-posee");
  ...
  useEffect(() => {
    console.log("Traversing carModelScene to find object names:");
    carModelScene.traverse((obj) => {
      console.log("CAR OBJECT:", obj.name);
    });
  }, [carModelScene]);
  ...
  return (
    <>
      ...
      <group ref={carModelGroupRef} visible={false}>
        ...
        <primitive ref={rearCamRef} object={rearCamScene} />
      </group>
    </>
  );
}

...
import FieldOfVision from "../TextComponents/FieldOfVision";
...
export default function LensOverlayContentDivs() {
    ...
    const ctx = gsap.context(() => {
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          ...
          else if (p > 0.1 && p < 0.041) setStage("s3");
          else if (p >= 0.137 && p < 0.237) setStage("s4");
          else if (p >= 0.272 && p < 0.338) setStage("s5");
          else if (p >= 0.418 && p < 0.450) setStage("s6");
          else if (p >= 0.460 && p < 0.490) setStage("s7");
          else if (p >= 0.585 && p < 0.595) setStage("s8");
          else if (p >= 0.667 && p < 0.752) setStage("s9");
          else if (p >= 0.831 && p < 0.950) setStage("s10");
  ...
  {stage === "s9" && (
    ...
    exit={{ opacity: 0, y: 10 }}
    transition={{ duration: 0.4 }}
    className={`fixed transition-all ease-in-out duration-300 w-full ${
      overlayPosition[stage || "default"]
    } pointer-events-none`}
  >
      {overlays.s9}

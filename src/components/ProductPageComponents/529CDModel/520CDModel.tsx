...
function CombinedModelAnimation({ onLoadComplete }: { onLoadComplete: () => void }) {
  const { scene: cameraModelScene } = useGLTF("/models/VREC_H520DC.glb");
  ...
  const cameraModelPosition = useRef(new THREE.Vector3(-11.95, 0, -5));
  const cameraModelScale = useRef(new THREE.Vector3(650, 650, 650));
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.addLabel("zoom-in")
        .to(cameraModelPosition.current, { x: -5, y: 0, z: 0.5 }, "zoom-in")

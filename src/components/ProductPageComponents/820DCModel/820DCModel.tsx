...
function Model1({ onModelReady }: { onModelReady: () => void }) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const customLensZPositions = [0.04, 0.055, 0.07, 0.08, 0.09, 0.10]; 
      // Animate target refs (not directly group)
      tl.addLabel("keyframe1")
        .to(targetPosition.current, { x: -5, y: 2.75, z: 0.5, ease: "none" }, "keyframe1")
        .to(targetScale.current, { x: 80, y: 80, z: 80, ease: "none" }, "keyframe1")
        .to(
      ...
      tl.addLabel("keyframe2")
        ...
        .to(targetScale.current, { x: 60, y: 60, z: 60, ease: "sine.inOut" }, "keyframe2")
        .to(
          targetRotation.current,
          {
            x: 0,
 ...
 const [showText, setShowText] = useState(false);
  return (
    <>
      ...
      <div id="model1-scroll-container" style={{ height: "1000vh" }}>
        <Canvas

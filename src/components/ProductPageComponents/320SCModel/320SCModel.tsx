...
function CombinedModelAnimation({ onLoadComplete }) {
  ...
  const cameraModelPosition = useRef(new THREE.Vector3(0, 0, 10));
...
loader.load("/images/Image.webp", (texture) => {
  ...
  if (displayMountRef.current) {
    ...
    plane.position.set(0, 0,-0.2); // Adjust if needed
  }

  // ✅ Video Setup
  const video = document.createElement("video");
  video.src = "/video/video.mp4"; // your video file
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
        ...
        scrollTrigger: {
          ...
          onUpdate: (self) => {
            ...
            if (imagePlaneRef.current) {
              const showImage = self.progress >= 0.3 && self.progress <= 0.30763;
              imagePlaneRef.current.visible = showImage || self.progress >= 0.364;
            
              // Video activation logic
              if (self.progress >= 0.364) {
                const { videoMap, videoEl } = imagePlaneRef.current.userData;
                if (videoMap && videoEl && imagePlaneRef.current.material.map !== videoMap) {
      ...
      tl.addLabel("zoom-out")
        ...
        .to(cameraModelPosition.current, { x: -6, y: 2.5, z: 0.5 }, "zoom-out")
        .to(
      ...
      tl.addLabel("rotate-and-scale")
        ...
        .to(cameraModelPosition.current, { x: 7, y: 2.5, z: 0 }, "rotate-and-scale")
        .to(
          cameraModelRotation.current,
          {
            x: 0,
      ...
      tl.addLabel("final-pose")
        .to(cameraModelPosition.current, { x: 0, y: 5, z: 0 }, "final-pose")
        .to(
          cameraModelRotation.current,
          {
            x: 0,
